...
class MercadoInteligenteGUI:
    ...
    def configurar_estilos(self):
        ...
        self.style.configure('TLabel', font=('Arial', 11), background='#f0f0f0', foreground='#333333')
        self.style.configure('Header.TLabel', font=('Arial', 14, 'bold'), background='#f0f0f0', foreground='#4CAF50')
        ...
        self.style.configure('Treeview.Heading', font=('Arial', 10, 'bold'), background='#4CAF50', foreground='white')
    ...
    def crear_interfaz(self):
        ...
        history_btn = ttk.Button(button_frame, text="Historial de Facturas", command=self.mostrar_historial)
        history_btn.pack(side=tk.RIGHT, padx=5)
        
        # Botón de salir
        exit_btn = ttk.Button(button_frame, text="Salir", command=self.root.destroy)
        ...
        self.label_subtotal = ttk.Label(totals_frame, text="Subtotal: $0.00", style='TLabel')
        self.label_subtotal.pack(side=tk.LEFT, padx=10)
        
        self.label_descuento = ttk.Label(totals_frame, text="Descuento: $0.00", style='TLabel')
        self.label_descuento.pack(side=tk.LEFT, padx=10)
        
        self.label_total = ttk.Label(totals_frame, text="Total: $0.00", font=('Arial', 12, 'bold'), foreground='#4CAF50')
    ...
    def agregar_producto(self):
        ...
        mensaje_descuento = ttk.Label(form_frame, 
                                     ...
                                     font=("Arial", 9), foreground='#4CAF50')
    ...
    def finalizar_compra(self):
        ...
        tk.Label(ticket_frame, text="MERCADO INTELIGENTE", 
                font=("Arial", 16, "bold"), bg="white", fg="#4CAF50").pack(pady=(0, 5))
        ...
        tk.Label(ticket_frame, 
                ...
                font=("Arial", 10), bg="white", fg="#333333").pack(pady=(0, 20))
        ...
        tk.Label(header_frame, text="Cant.", width=6, anchor="w", 
                font=("Arial", 10, "bold"), bg="white", fg="#4CAF50").pack(side=tk.LEFT)
        ...
        # Línea divisoria
        tk.Frame(scrollable_frame, height=1, bg="#4CAF50").pack(fill=tk.X, pady=(0, 5))
        ...
        for producto in self.carrito:
            ...
            tk.Label(item_frame, text=str(producto['cantidad']), width=6, anchor="w", 
                    font=("Arial", 10), bg="white", fg="#333333").pack(side=tk.LEFT)
        ...
        # Línea divisoria
        tk.Frame(scrollable_frame, height=1, bg="#4CAF50").pack(fill=tk.X, pady=10)
        ...
        tk.Label(totals_frame, text=f"Subtotal ({total_productos} productos):", 
                anchor="e", font=("Arial", 10), bg="white", fg="#333333").pack(side=tk.LEFT, expand=True, fill=tk.X)
        tk.Label(totals_frame, text=f"${total_subtotal:.2f}", width=10, 
                anchor="e", font=("Arial", 10), bg="white", fg="#333333").pack(side=tk.RIGHT)
        ...
        tk.Label(final_frame, text="TOTAL A PAGAR:", 
                anchor="e", font=("Arial", 12, "bold"), bg="white", fg="#4CAF50").pack(side=tk.LEFT, expand=True, fill=tk.X)
        tk.Label(final_frame, text=f"${total_pagar:.2f}", width=10, 
                anchor="e", font=("Arial", 12, "bold"), bg="white", fg="#4CAF50").pack(side=tk.RIGHT)
        
        # Mensaje final
        tk.Frame(scrollable_frame, height=1, bg="#4CAF50").pack(fill=tk.X, pady=10)
        tk.Label(scrollable_frame, text="¡Gracias por su compra!", 
                font=("Arial", 12, "bold"), bg="white", fg="#4CAF50").pack(pady=10)
    ...
    def guardar_ticket(self, ticket_window):
        ...
        for producto in self.carrito:
            contenido.append(f"{producto['cantidad']:<6} {producto['nombre']:<20} ${producto['precio']:>7.2f} ${producto['subtotal']:>8.2f}")
        ...
        contenido.append(f"Subtotal ({total_productos} productos): ${total_subtotal:.2f}")
        contenido.append(f"Descuento aplicado: ${total_descuento:.2f}")
        contenido.append(f"TOTAL A PAGAR: ${total_pagar:.2f}")
        contenido.append("")
        ...
        nombre_archivo = f"ticket_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        pdf.output(nombre_archivo)
        
        messagebox.showinfo("Ticket Guardado", f"El ticket se ha guardado correctamente en '{nombre_archivo}'")
    
    def mostrar_historial(self):
        """Muestra una ventana con el historial de facturas"""
        # Crear ventana de historial
        historial_window = tk.Toplevel(self.root)
        historial_window.title("Historial de Facturas")
        historial_window.geometry("600x400")
        historial_window.configure(bg="#f0f0f0")
        
        # Frame para la lista de facturas
        list_frame = ttk.Frame(historial_window, padding="10")
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        # Obtener la lista de archivos de tickets
        tickets = glob.glob("ticket_*.pdf")
        tickets.sort(reverse=True)  # Ordenar de más reciente a más antiguo
        
        # Crear un Treeview para mostrar los tickets
        ...
        # Agregar tickets al Treeview
        for ticket in tickets:
            nombre = os.path.basename(ticket)
            fecha_creacion = datetime.datetime.fromtimestamp(os.path.getctime(ticket)).strftime('%d/%m/%Y %H:%M:%S')
            tree.insert('', 'end', values=(nombre, fecha_creacion))
        
        # Empaquetar Treeview
        tree.pack(fill=tk.BOTH, expand=True)
        
        # Botón para abrir el ticket seleccionado
        def abrir_ticket():
            seleccion = tree.selection()
            if not seleccion:
                messagebox.showinfo("Información", "Seleccione un ticket para abrir")
                return
            
            # Obtener el nombre del archivo seleccionado
            nombre_archivo = tree.item(seleccion[0])['values'][0]
            
            # Abrir el archivo PDF
            try:
                os.startfile(nombre_archivo)  # Abre el archivo con el programa predeterminado
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo abrir el ticket: {e}")
        
        # Botón para abrir el ticket seleccionado
        open_btn = ttk.Button(list_frame, text="Abrir Ticket Seleccionado", command=abrir_ticket)
